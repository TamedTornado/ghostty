...
struct Uniforms {
  ...
  ushort2 grid_size;
  float4 grid_padding;
  float2 smooth_scroll_offset;
  float2 image_scroll_offset;
...
fragment float4 cell_bg_fragment(
  ...
  int2 grid_pos = int2(floor((in.position.xy - uniforms.grid_padding.wx - uniforms.smooth_scroll_offset) / uniforms.cell_size));
...
vertex CellTextVertexOut cell_text_vertex(
  ...
  float2 cell_pos = uniforms.cell_size * float2(in.grid_pos) + uniforms.smooth_scroll_offset;
...
vertex ImageVertexOut image_vertex(
  ...
  float2 image_pos = (uniforms.cell_size * in.grid_pos) + in.cell_offset + uniforms.image_scroll_offset;
